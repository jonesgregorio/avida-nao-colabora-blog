...
import { Search, Clock, ArrowRight, X, BookOpen, Lock, Compass, PlayCircle } from 'lucide-react'
...
export default function Articles({ onSelectArticle, user, profile, onNavigateDiary, onNavigatePricing }: ArticlesProps) {
  ...
  const filtered = useMemo(() => {
    let result = catalog
    if (mode === 'read') result = result.filter(it => !isPractice(it))
  ...
  return (
    <section id="articles" className="max-w-6xl mx-auto px-4 sm:px-6 py-7 sm:py-10">
      <header className="mb-6 sm:mb-8 max-w-3xl">
        ...
        <p className="mt-2 text-ink-soft max-w-2xl leading-relaxed">Escolha uma leitura para compreender melhor um tema ou uma prática para fazer algo no seu ritmo.</p>
      </header>
      ...
      <div className="mb-7 grid grid-cols-3 rounded-2xl border border-line bg-paper-soft p-1" role="tablist" aria-label="Tipo de conteúdo">
        {([
          ...
          return <button key={key} type="button" role="tab" aria-selected={active} onClick={() => setMode(key)} className={`inline-flex items-center justify-center gap-1.5 rounded-xl px-3 py-2.5 text-sm font-medium transition-colors ${active ? 'bg-white text-forest-900 shadow-sm' : 'text-ink-soft hover:text-forest-900'}`}><Icon className="w-4 h-4" />{label}</button>
        })}
      </div>
      ...
      {user && <div className="mb-9"><RecommendedContent user={user} profile={profile} catalog={catalog.length ? catalog : null} source="guided_page" limit={3} showEmpty onOpen={onSelectArticle} onCheckin={onNavigateDiary} onDiary={onNavigateDiary} onSeeAll={() => { setFilter('Todos'); setSearch(''); setMode('all') }} /></div>}

      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-2 mb-4">
        <div><h2 className="font-serif text-xl sm:text-2xl text-forest-900">{mode === 'read' ? 'Leituras' : mode === 'practice' ? 'Práticas guiadas' : 'Biblioteca'}</h2><p className="text-sm text-ink-soft mt-1">{mode === 'read' ? 'Textos para entender um tema com calma.' : mode === 'practice' ? 'Exercícios e pausas para fazer no seu ritmo.' : 'Explore por tema ou busque pelo que faz sentido agora.'}</p></div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 text-ink-soft"><p className="mb-2 font-medium text-forest-900">Nenhum conteúdo encontrado.</p>{!isDefault ? <button onClick={() => { setFilter('Todos'); setSearch(''); setMode('all') }} className="text-sm text-forest-700 underline underline-offset-2">Ver todos os conteúdos</button> : <p className="text-sm">Novos conteúdos são publicados regularmente. Volte em breve.</p>}</div>
      ) : (
