...
import { fetchReadSlugs } from '../lib/readingProgress'
...
type LibraryMode = 'all' | 'history' | 'practice'
...
export default function Articles({ onSelectArticle, user, profile, onNavigateDiary, onNavigatePricing }: ArticlesProps) {
  ...
  useEffect(() => {
    let live = true
    fetchReadSlugs(user?.id).then(slugs => { if (live) setReadSlugs(slugs) })
    return () => { live = false }
  }, [user?.id])

  const filtered = useMemo(() => {
    let result = catalog
    if (mode === 'history') result = result.filter(it => !isPractice(it) && Boolean(it.slug && readSlugs.has(it.slug)))
    ...
    return result
  }, [catalog, filter, search, filters, mode, readSlugs])
  ...
  const heading = mode === 'history' ? 'Minhas leituras' : mode === 'practice' ? 'Práticas guiadas' : 'Biblioteca completa'
  const description = mode === 'history'
    ? 'Artigos que você já abriu ficam reunidos aqui para consultar novamente.'
    : mode === 'practice'
      ? 'Exercícios e pausas para fazer no seu ritmo.'
      : 'Leituras e práticas em um só lugar. Use temas e busca para encontrar o que precisa.'
  ...
  return (
    <section id="articles" className="max-w-6xl mx-auto px-4 sm:px-6 py-7 sm:py-10">
      <header className="mb-6 sm:mb-8 max-w-3xl">
        ...
        <p className="mt-2 text-ink-soft max-w-2xl leading-relaxed">Explore a biblioteca, retome artigos que já abriu ou escolha uma prática guiada.</p>
      </header>

      <div className="mb-7 grid grid-cols-3 gap-1 rounded-[22px] border border-line bg-paper-soft p-1.5 shadow-[0_1px_0_rgba(15,47,37,0.02)]" role="tablist" aria-label="Tipo de conteúdo">
        {([
          ['all', 'Todos', Compass],
          ['history', 'Minhas leituras', History],
          ['practice', 'Praticar', PlayCircle],
        ] as const).map(([key, label, Icon]) => {
          const active = mode === key
          return <button key={key} type="button" role="tab" aria-selected={active} onClick={() => setMode(key)} className={`inline-flex min-h-11 items-center justify-center gap-1.5 rounded-2xl px-2 sm:px-3 py-2.5 text-xs sm:text-sm font-medium transition-all ${active ? 'bg-white text-forest-900 shadow-sm ring-1 ring-line/70' : 'text-ink-soft hover:text-forest-900 hover:bg-white/55'}`}><Icon className="w-4 h-4 flex-shrink-0" /><span className="truncate">{label}</span></button>
        })}
      </div>

      {mode === 'history' && !user && <div className="mb-7 rounded-2xl border border-line bg-mint/35 p-4 text-sm text-forest-900">Entre na sua conta para ver os artigos que você já abriu.</div>}

      <a href="/guias" className="mb-8 flex items-center gap-4 rounded-[22px] border border-line bg-white px-4 sm:px-5 py-4 sm:py-5 transition-all hover:border-forest-200 hover:shadow-sm">
      ...
      {user && mode === 'all' && <div className="mb-9"><RecommendedContent user={user} profile={profile} catalog={catalog.length ? catalog : null} source="guided_page" limit={3} showEmpty onOpen={onSelectArticle} onCheckin={onNavigateDiary} onDiary={onNavigateDiary} onSeeAll={() => { setFilter('Todos'); setSearch(''); setMode('all') }} /></div>}

      <div className="mb-5 flex flex-col gap-3 border-b border-line pb-5 sm:flex-row sm:items-end sm:justify-between">
        <div><h2 className="font-serif text-xl sm:text-2xl text-forest-900">{heading}</h2><p className="text-sm text-ink-soft mt-1 max-w-2xl">{description}</p></div>
        <span className="text-xs text-ink-soft whitespace-nowrap">{filtered.length} {filtered.length === 1 ? 'conteúdo' : 'conteúdos'}</span>
      </div>

      <div className="relative mb-4">
        <Search size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-ink-soft pointer-events-none" />
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar conteúdo, emoção ou tema" aria-label="Buscar conteúdo, emoção ou tema" className="w-full pl-11 pr-11 py-3.5 rounded-2xl border border-line bg-white text-ink placeholder:text-ink-soft/70 focus:outline-none focus-visible:ring-2 focus-visible:ring-forest-300" />
      ...
      {loading ? (
        ...
        <div className="rounded-[26px] border border-dashed border-line bg-paper-soft/45 px-5 py-14 text-center text-ink-soft"><p className="mb-2 font-medium text-forest-900">{mode === 'history' ? 'Você ainda não abriu nenhum artigo desta seleção.' : 'Nenhum conteúdo encontrado.'}</p><p className="mx-auto mb-4 max-w-lg text-sm">{mode === 'history' ? 'Ao abrir uma leitura, ela passa a aparecer aqui para você encontrar de novo com facilidade.' : 'Tente outro tema ou limpe os filtros para voltar à biblioteca.'}</p>{!isDefault && <button onClick={() => { setFilter('Todos'); setSearch(''); setMode('all') }} className="text-sm text-forest-700 underline underline-offset-2">Ver todos os conteúdos</button>}</div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {filtered.map(it => <LibraryCard key={it.id} item={it} locked={isContentLocked(it.plan_required, plan, !!user)} onOpen={() => handleSelect(it)} onUpgrade={onNavigatePricing} />)}
        </div>
      )}
    </section>
  )
