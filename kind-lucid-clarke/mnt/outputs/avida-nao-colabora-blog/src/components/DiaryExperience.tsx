...
import {
  ...
  Loader2, Maximize2, Mic, Minimize2, Save, Sparkles,
  SlidersHorizontal, X,
} from 'lucide-react'
...
import DiaryHistorySection, { type DiaryHistoryFilter, type DiaryHistoryPeriodFilter } from './DiaryHistorySection'
...
export default function DiaryExperienceRefined({ user, plan, onBack, onNavigatePricing, initialMood, promptContext, onClearPromptContext, onOpenArticle }: DiaryExperienceProps) {
  ...
  const [filter, setFilter] = useState<DiaryHistoryFilter>('all')
  const [periodFilter, setPeriodFilter] = useState<DiaryHistoryPeriodFilter>('all')
  ...
  const startDeepening = async (question?: string) => {
    ...
    if (!entry) return
    if (!isPlus) { setError('Aprofundamentos do Diário estão disponíveis a partir do plano Plus.'); return }
  ...
  const contentWidth = focusMode ? 'max-w-3xl py-8 sm:py-12' : 'max-w-6xl py-6 sm:py-9'
  const formattedToday = new Date().toLocaleDateString('pt-BR', { weekday: 'long', day: '2-digit', month: 'long' })
      ...
      <div className={`${contentWidth} mx-auto px-4 sm:px-6`}>
        ...
        {tab === 'write' ? (
          <div className={focusMode ? '' : 'relative rounded-[2rem] border border-[#cfc2a8] bg-[#dfd3bc] p-2.5 sm:p-4 shadow-[0_24px_60px_rgba(53,69,54,0.14)]'}>
            <div className={focusMode ? '' : 'relative overflow-hidden rounded-[1.5rem] border border-[#ded3bf] bg-[#fffdf8]'}>
              {!focusMode && <div aria-hidden className="pointer-events-none absolute inset-y-4 left-1/2 z-20 hidden w-7 -translate-x-1/2 rounded-full bg-[linear-gradient(90deg,rgba(101,81,54,0.13),rgba(255,255,255,0.72)_42%,rgba(255,255,255,0.88)_50%,rgba(101,81,54,0.16)_58%,rgba(101,81,54,0.05))] shadow-[0_0_18px_rgba(84,68,45,0.18)] lg:block" />}
              <div className={focusMode ? '' : 'grid lg:grid-cols-2'}>
                ...
                <main className={focusMode ? 'min-w-0' : 'min-w-0 p-4 sm:p-7 lg:pl-11'}>
                  <div className={focusMode ? '' : 'mx-auto max-w-3xl'}>
                    {mode === 'main-saved' && todayMain ? (
                      <section className="rounded-3xl border border-line bg-paper-soft p-6 sm:p-8"><p className="text-xs uppercase tracking-[0.14em] text-forest-600">Diário de hoje</p><h2 className="font-serif text-2xl text-forest-900 mt-1">Você já colocou algo no papel hoje.</h2><p className="text-sm text-ink-soft mt-2 leading-relaxed">{isPlus ? (todayDeepened ? 'Você já usou os 3 aprofundamentos disponíveis para o Diário de hoje. O registro continua guardado exatamente como ficou.' : `Se algo ainda ficou sem palavras, você pode aprofundar este mesmo Diário mais ${deepeningsRemaining} ${deepeningsRemaining === 1 ? 'vez' : 'vezes'} hoje.`) : 'O registro continua guardado exatamente como ficou.'}</p>{isPlus && <div className="mt-3 inline-flex rounded-full border border-line bg-white px-3 py-1 text-xs text-forest-700">{todayDeepeningCount} de {MAX_DEEPENINGS_PER_DAY} aprofundamentos usados hoje</div>}<blockquote className="mt-5 border-l-2 border-forest-300 pl-4 text-sm text-ink-soft whitespace-pre-line line-clamp-5">{todayMain.text || 'Registro sem texto.'}</blockquote><div className="flex flex-wrap gap-2 mt-5">{isPlus && !todayDeepened && <button onClick={() => void startDeepening()} className="rounded-xl bg-forest-900 text-white px-4 py-2.5 text-sm font-medium">Aprofundar meu registro</button>}{!isPlus && onNavigatePricing && <button onClick={onNavigatePricing} className="text-xs text-forest-700 font-medium">Aprofundamentos do Diário — disponível no Plus</button>}</div></section>
                    ) : (
