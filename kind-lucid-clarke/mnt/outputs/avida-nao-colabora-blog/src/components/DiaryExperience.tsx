    ...
    return (
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10 sm:py-16">
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-3">
          ...
          {saved.kind === 'diary' && isEssential && !todayDeepened && <button onClick={() => void askFollowUp()} className="rounded-2xl bg-forest-900 text-white px-5 py-2.5 text-sm font-medium inline-flex items-center gap-2"><Sparkles className="w-4 h-4" /> Quero responder à pergunta</button>}
    ...
    <div className={shell}>
      <div className={`${focusMode ? 'max-w-3xl py-8 sm:py-12' : 'max-w-6xl py-6 sm:py-9'} mx-auto px-4 sm:px-6`}>
        <header className="flex items-start justify-between gap-4 mb-6">
          ...
          <button onClick={() => setFocusMode(v => !v)} className="flex-shrink-0 rounded-xl border border-line bg-white p-2.5 text-forest-800" title={focusMode ? 'Sair do modo foco' : 'Modo foco'}>{focusMode ? <Minimize2 className="w-4 h-4" /> : <Maximize2 className="w-4 h-4" />}</button>
        </header>
        ...
        {tab === 'write' ? (
                ...
                <section className={`${focusMode ? 'bg-transparent' : 'bg-paper-soft border border-line rounded-[2rem]'} p-4 sm:p-7`}>
                  ...
                  {mode === 'quick' ? (
                    <>
                      ...
                      <textarea value={quickNote} onChange={e => setQuickNote(e.target.value)} rows={4} placeholder="Uma frase já basta…" className="mt-2 w-full rounded-2xl border border-line bg-white px-4 py-4 text-base resize-none focus:outline-none focus-visible:ring-2 focus-visible:ring-forest-300" />
                    </>
                  ) : (
                    <>
                      {!editingEntryId && !helperPrompt && (
                        <div className="flex flex-wrap gap-2 mb-4">
                          <button type="button" onClick={() => { setHelperPrompt(''); setStarterOpen(false); editorRef.current?.focus() }} className="rounded-xl border border-line bg-white px-3.5 py-2 text-sm text-forest-900 inline-flex items-center gap-2"><PenLine className="w-4 h-4" /> Só quero escrever</button>
                          <button type="button" onClick={() => void requestStartHelp()} disabled={helperLoading} className="rounded-xl bg-mint border border-forest-100 px-3.5 py-2 text-sm text-forest-900 inline-flex items-center gap-2 disabled:opacity-60">{helperLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Sparkles className="w-4 h-4" />} Me ajude a começar</button>
                          <button type="button" onClick={() => setStarterOpen(v => !v)} className="rounded-xl px-3.5 py-2 text-sm text-ink-soft">Não sei o que escrever</button>
                        </div>
                      )}
                      {starterOpen && <div className="mb-4 rounded-2xl border border-line bg-white p-4"><p className="text-sm text-forest-900 font-medium">Escolha só o que mais se aproxima:</p><div className="flex flex-wrap gap-2 mt-3">{[['crowded','Minha cabeça está cheia'],['happened','Aconteceu alguma coisa hoje'],['worried','Estou preocupado com algo'],['good','Estou bem e quero registrar'],['unclear','Nem sei explicar']].map(([key,label]) => <button key={key} onClick={() => starterChoice(key)} className="rounded-full border border-line px-3 py-1.5 text-xs text-forest-800 hover:bg-mint/40">{label}</button>)}</div></div>}
                      {helperPrompt && <div className="mb-4 rounded-2xl border border-forest-100 bg-mint/45 p-4 flex gap-3"><Sparkles className="w-4 h-4 text-forest-600 mt-0.5 flex-shrink-0" /><div className="flex-1"><p className="text-sm text-forest-900">{helperPrompt}</p><button type="button" onClick={() => void requestStartHelp()} className="text-xs text-forest-700 mt-2">Quero outra pergunta</button></div><button onClick={() => setHelperPrompt('')} className="text-ink-soft"><X className="w-4 h-4" /></button></div>}

                      <div className={`${focusMode ? 'min-h-[58vh]' : ''}`}>
                        <textarea ref={editorRef} value={draft} onChange={e => setDraft(e.target.value)} rows={focusMode ? 18 : 10} placeholder="Escreva do seu jeito. Pode ser uma frase, um desabafo ou tudo que estiver na sua cabeça…" className={`w-full bg-transparent px-1 py-3 font-serif text-lg sm:text-xl leading-relaxed resize-none focus:outline-none placeholder:text-ink-soft/55 ${focusMode ? 'min-h-[55vh]' : ''}`} />
                        <div className="border-t border-line/70 pt-3 flex flex-wrap items-center justify-between gap-2">
                          <div className="flex flex-wrap gap-2">
                            ...
                            {isEssential && draft.trim().length >= 10 && <button type="button" onClick={() => void organizeWriting()} disabled={organizing} className="rounded-xl bg-white border border-line px-3 py-2 text-xs text-forest-800 inline-flex items-center gap-1.5 disabled:opacity-60">{organizing ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Sparkles className="w-3.5 h-3.5" />} Organizar minha escrita</button>}
                          </div>
                          <span className="text-[11px] text-ink-soft">{draft.length} caracteres</span>
                        </div>
                      </div>

                      {organizedCandidate && <div className="mt-4 rounded-2xl border border-forest-100 bg-mint/35 p-4"><p className="text-xs font-semibold text-forest-700">Uma versão organizada, sem mudar o sentido</p><p className="text-sm text-ink mt-2 whitespace-pre-line leading-relaxed">{organizedCandidate}</p><div className="flex gap-2 mt-3"><button onClick={() => { setDraft(organizedCandidate); setOrganizedCandidate('') }} className="rounded-xl bg-forest-900 text-white px-3 py-2 text-xs">Usar esta versão</button><button onClick={() => setOrganizedCandidate('')} className="rounded-xl border border-line bg-white px-3 py-2 text-xs">Manter meu texto</button></div></div>}
                      ...
                      <div className="mt-5 flex items-center justify-between gap-3 flex-wrap">
                        ...
                        <label className="flex items-center gap-2 text-xs text-ink-soft cursor-pointer"><input type="checkbox" checked={!aiAllowed} onChange={e => setAiAllowed(!e.target.checked)} className="accent-forest-700" /> Não analisar este registro com IA</label>
                      </div>
          ...
          <section ref={historyRef}>
            ...
            <div className="flex items-center justify-between gap-3 flex-wrap mb-4"><div className="flex flex-wrap gap-2">{(['all','diary','checkin','questionnaire'] as Filter[]).map(f => <button key={f} onClick={() => setFilter(f)} className={`rounded-full border px-3 py-1.5 text-xs ${filter === f ? 'bg-forest-900 text-white border-forest-900' : 'bg-white border-line text-ink-soft'}`}>{f === 'all' ? 'Tudo' : f === 'diary' ? 'Diários' : f === 'checkin' ? 'Check-ins' : 'Avaliações'}</button>)}<select value={periodFilter} onChange={e => setPeriodFilter(e.target.value as PeriodFilter)} className="rounded-full border border-line bg-white px-3 py-1.5 text-xs text-ink-soft"><option value="all">Todos os períodos</option><option value="7d">Últimos 7 dias</option><option value="30d">Últimos 30 dias</option><option value="month">Mês atual</option></select></div><button onClick={() => void fetchEntries(true)} className="p-2 text-ink-soft" title="Atualizar"><RefreshCw className="w-4 h-4" /></button></div>
