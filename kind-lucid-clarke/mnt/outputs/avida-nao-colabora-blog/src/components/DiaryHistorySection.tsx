...
type MobilePage = 'calendar' | 'day'

interface Props {
  sectionRef: Ref<HTMLElement>; entries: DiaryHistoryEntry[]; monthRows: DiaryHistoryEntry[]; today: string
  exportPdfEnabled: boolean; exporting: boolean; filter: DiaryHistoryFilter; periodFilter: DiaryHistoryPeriodFilter
  expandedId: string | null; loading: boolean; loadingMore: boolean; hasMore: boolean
  getMoodMeta: (value: string | number | null | undefined) => { emoji: string; label: string }
  onExport: () => void; onFilterChange: (filter: DiaryHistoryFilter) => void
  onPeriodFilterChange: (period: DiaryHistoryPeriodFilter) => void; onExpandedChange: (id: string | null) => void
  onRefresh: () => void; onLoadMore: () => void
}

function dayLabel(date: string) { return new Date(`${date}T12:00:00`).toLocaleDateString('pt-BR', { weekday: 'long', day: '2-digit', month: 'long', year: 'numeric' }) }
...
function shiftMonth(monthKey: string, amount: number) { const [year, month] = monthKey.split('-').map(Number); const next = new Date(year, month - 1 + amount, 1); return `${next.getFullYear()}-${String(next.getMonth() + 1).padStart(2, '0')}` }
function monthTitle(monthKey: string) { return new Date(`${monthKey}-01T12:00:00`).toLocaleDateString('pt-BR', { month: 'long', year: 'numeric' }) }
...
export default function DiaryHistorySection(p: Props) {
  ...
  const [mobilePage, setMobilePage] = useState<MobilePage>('calendar')

  void p.filter; void p.periodFilter; void p.loading; void p.loadingMore; void p.hasMore; void p.onFilterChange; void p.onPeriodFilterChange; void p.onLoadMore

  useEffect(() => { if (viewMonth !== currentMonth) return; setVisibleRows(monthRows) }, [currentMonth, monthRows, viewMonth])
  const fetchViewedMonth = useCallback(async (monthKey: string) => {
    if (monthKey === currentMonth) { p.onRefresh(); setVisibleRows(monthRows); return }
    if (!userId) { setVisibleRows([]); return }
    setMonthLoading(true)
    try { setVisibleRows(await loadDiaryMonth(userId, monthKey) as DiaryHistoryEntry[]) } finally { setMonthLoading(false) }
  }, [currentMonth, monthRows, p, userId])
  useEffect(() => {
    if (viewMonth === currentMonth) return
    if (!userId) { setVisibleRows([]); return }
    let cancelled = false; setMonthLoading(true)
    void loadDiaryMonth(userId, viewMonth).then(rows => { if (!cancelled) setVisibleRows(rows as DiaryHistoryEntry[]) }).finally(() => { if (!cancelled) setMonthLoading(false) })
    return () => { cancelled = true }
  }, [currentMonth, userId, viewMonth])

  const groupedHistory = useMemo(() => { const map = new Map<string, DiaryHistoryEntry[]>(); for (const entry of visibleRows) { const date = String(entry.date || '').slice(0, 10); map.set(date, [...(map.get(date) || []), entry]) } return map }, [visibleRows])
  useEffect(() => { const dates = [...groupedHistory.keys()].sort(); if (viewMonth === currentMonth) { setSelectedDate(today); return } setSelectedDate(dates[dates.length - 1] || `${viewMonth}-01`) }, [currentMonth, groupedHistory, today, viewMonth])
  useEffect(() => { setMobilePage('calendar') }, [viewMonth])

  const moodByDay = useMemo(() => { const map = new Map<string, string>(); for (const row of visibleRows) { if (!hasMoodValue(row.mood)) continue; map.set(String(row.date || '').slice(0, 10), getMoodMeta(row.mood).emoji) } return map }, [getMoodMeta, visibleRows])
  ...
  const selectMobileDate = (date: string) => { setSelectedDate(date); setMobilePage('day'); onExpandedChange(null) }
  ...
  const dayPage = (mobile = false) => <>
    <div className="flex items-start justify-between gap-3">
      <div>{mobile && <button type="button" onClick={() => setMobilePage('calendar')} className="mb-3 inline-flex items-center gap-1 text-xs font-medium text-forest-700"><ChevronLeft className="h-3.5 w-3.5" /> Voltar ao calendário</button>}<p className="text-[10px] uppercase tracking-[0.16em] text-forest-600 capitalize">{dayLabel(selectedDate)}</p><h3 className="mt-1 font-serif text-2xl text-forest-900">{selectedRows.length ? 'Página deste dia' : 'Uma página em branco'}</h3></div>
  ...
  return <section ref={sectionRef}>
    {/* Mobile: one notebook page at a time. Desktop below remains the same two-page book. */}
    <div data-diary-mobile-page className="lg:hidden relative mb-5 rounded-[1.7rem] border border-[#cfc2a8] bg-[#dfd3bc] p-2.5 shadow-[0_18px_45px_rgba(53,69,54,0.14)]">
      <div className="relative min-h-[620px] overflow-hidden rounded-[1.35rem] border border-[#ded3bf] bg-[#fffdf8] shadow-[inset_7px_0_16px_-16px_rgba(70,52,30,0.75)]">
        <div aria-hidden className="pointer-events-none absolute inset-y-4 left-0 w-1.5 rounded-r-full bg-[linear-gradient(90deg,rgba(99,75,43,0.22),rgba(255,255,255,0.7))]" />
        <div className="p-5">{mobilePage === 'calendar' ? calendar(true) : dayPage(true)}</div>
        <div className="pointer-events-none absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5" aria-hidden><span className={`h-1.5 rounded-full transition-all ${mobilePage === 'calendar' ? 'w-5 bg-forest-700' : 'w-1.5 bg-forest-200'}`} /><span className={`h-1.5 rounded-full transition-all ${mobilePage === 'day' ? 'w-5 bg-forest-700' : 'w-1.5 bg-forest-200'}`} /></div>
      </div>
    </div>

    <div className="hidden lg:block relative mb-5 rounded-[2rem] border border-[#cfc2a8] bg-[#dfd3bc] p-2.5 sm:p-4 shadow-[0_24px_60px_rgba(53,69,54,0.14)]">
      <div className="relative overflow-hidden rounded-[1.5rem] border border-[#ded3bf] bg-[#fffdf8]">
        ...
        <div className="grid lg:grid-cols-2">
          <div className="relative min-h-[540px] border-b border-[#e1d7c5] p-5 sm:p-7 lg:border-b-0 lg:border-r lg:border-[#d8cbb5] lg:pr-10">{calendar(false)}</div>
          <div className="relative min-h-[540px] p-5 sm:p-7 lg:pl-10">{dayPage(false)}</div>
        </div>
      </div>
    </div>
  </section>
