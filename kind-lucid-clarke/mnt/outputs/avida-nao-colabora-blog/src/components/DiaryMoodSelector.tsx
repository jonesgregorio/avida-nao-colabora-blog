...
export default function DiaryMoodSelector({ selectedKey, otherLabel, onSelect, onOtherLabelChange, optional = false }: DiaryMoodSelectorProps) {
  ...
  return <div className="mb-5">
    <p className="text-sm font-semibold text-forest-900 mb-1">Como este momento está para você? {optional && <span className="font-normal text-ink-soft">(opcional)</span>}</p>
    <p className="text-xs text-ink-soft mb-3">Aqui você marca um estado emocional. É diferente da pergunta da página inicial sobre como o dia, no geral, colaborou.</p>
    <div className="flex flex-wrap gap-2">
      ...
      <button type="button" onClick={() => setShowAll(value => !value)} className="inline-flex items-center gap-1.5 rounded-full border border-line bg-white px-3 py-2 text-sm text-forest-800 hover:border-forest-300" aria-expanded={expanded}>
        {expanded ? <ChevronUp className="w-3.5 h-3.5" /> : <ChevronDown className="w-3.5 h-3.5" />}{expanded ? 'Menos estados' : 'Outros estados'}
      </button>
    </div>
