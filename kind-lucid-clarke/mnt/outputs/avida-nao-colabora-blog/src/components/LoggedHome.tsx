...
const COLLABORATION = [
  ...
  { score: 2, emoji: '😕', label: 'Quase nada' },
  { score: 3, emoji: '😐', label: 'Mais ou menos' },
  { score: 4, emoji: '🙂', label: 'Até que sim' },
...
export default function LoggedHome({ user, profile, onNavigate }: LoggedHomeProps) {
  ...
  useEffect(() => {
    ...
    void supabase.from('daily_life_collaboration').select('score').eq('user_id', user.id).eq('date', todayKey()).maybeSingle().then(({ data }) => {
      if (active) setScore(typeof data?.score === 'number' ? data.score : null)
    })
    return () => { active = false }
  }, [user])
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-5 sm:pt-7 lg:pt-8">
      <section className="relative overflow-hidden rounded-[30px] border border-line bg-gradient-to-br from-mint via-paper-soft to-sand-50 p-5 sm:p-7 lg:p-8">
        ...
        <div className="relative">
          ...
          <p className="text-sm sm:text-base text-ink-soft mt-3 max-w-2xl leading-relaxed">Pense no dia como um todo. Não é uma emoção e não precisa estar tudo bem — escolha apenas a resposta que chega mais perto.</p>
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-2.5 mt-6" aria-label="Quanto a vida colaborou hoje">
            {COLLABORATION.map(item => { const active = score === item.score; return <button key={item.score} type="button" disabled={saving} onClick={() => void chooseScore(item.score)} aria-pressed={active} className={`relative rounded-2xl border px-3 py-4 text-center transition-all ${active ? 'border-forest-900 bg-forest-900 text-white shadow-sm' : 'border-line bg-white/80 text-forest-900 hover:border-forest-300 hover:bg-white'} disabled:opacity-60`}>
              {active && <Check className="absolute right-2 top-2 w-3.5 h-3.5" />}<span className="block text-2xl" aria-hidden>{item.emoji}</span><span className="block text-sm font-medium mt-1.5">{item.label}</span>
            </button> })}
          </div>
          {selected && <div className="mt-6 rounded-3xl border border-white bg-white/70 p-4 sm:p-5">
            <p className="text-sm font-semibold text-forest-900">Você marcou: {selected.emoji} {selected.label}</p>
            <p className="text-sm text-ink-soft mt-1">Quer dizer como isso apareceu em você? É opcional.</p>
            {showFeelings && <div className="flex flex-wrap gap-2 mt-4">{featuredMoods.map(mood => <MoodChip key={mood.key} mood={mood} active={false} onClick={() => onNavigate(`diary?mood=${mood.key}`)} />)}</div>}
            <div className="flex flex-wrap gap-2 mt-4">
              <button type="button" onClick={() => onNavigate('diary')} className="rounded-2xl bg-forest-900 px-4 py-2.5 text-sm font-medium text-white">Quero escrever</button>
              <button type="button" onClick={() => setShowFeelings(value => !value)} className="rounded-2xl border border-line bg-white px-4 py-2.5 text-sm font-medium text-forest-800">{showFeelings ? 'Ocultar sentimentos' : 'Registrar como estou'}</button>
            </div>
          </div>}
        </div>
      </section>
    </div>
    <style>{`.avnc-legacy-home > div > section:first-child { display: none !important; } .avnc-legacy-home > div { padding-top: 0 !important; }`}</style>
