...
export default function MyGardenPage({userId}:Props){
  ...
  const level=Math.max(1,Math.floor(growth/12)+1)
  ...
  return <div className="min-h-full bg-[#fbf7ef] text-forest-950"><div className="mx-auto max-w-[1180px] px-4 py-7 sm:px-6 lg:px-8">
    ...
    <section className={`relative mt-6 min-h-[470px] overflow-hidden rounded-[34px] border border-[#dcd3c4] bg-gradient-to-b ${theme.ground} shadow-[0_18px_50px_rgba(36,70,49,.10)]`} aria-label={`${cycleLabel(currentCycle)}: ${theme.name}`}>
      ...
      <div className="absolute left-[8%] top-[13%] text-[110px] sm:text-[150px] drop-shadow-sm select-none" aria-hidden>{pick(['🌳','🌲','🌿','🌳'],currentCycle)}</div>
      {cycleProgress>=46&&<div className="absolute right-[6%] top-[12%] text-[105px] sm:text-[145px] select-none" aria-hidden>{pick(['🌸','🌳','🌻','🌲'],currentCycle+2)}</div>}
      <div className="absolute left-[2%] right-[2%] bottom-0 h-[48%] rounded-t-[50%] bg-[#b9ce9d]/70"/><div className="absolute left-[17%] bottom-[3%] h-[42%] w-[23%] rotate-[18deg] rounded-[50%] bg-[#e7d9b8] opacity-80"/>
      {cycleProgress>=55&&<div className="absolute bottom-[9%] right-[22%] h-24 w-44 rounded-[50%] border-4 border-white/50 bg-[#8bb6ac] shadow-inner"><Waves className="m-auto mt-7 text-white/80"/></div>}
      <div className="absolute bottom-[5%] left-[5%] right-[5%] flex flex-wrap items-end justify-center gap-1 text-4xl sm:text-5xl select-none" aria-hidden>{Array.from({length:7+lush*3},(_,i)=><span key={i}>{pick(['🌿','🌼','🌱','🌷','🌾','🪻'],currentCycle+i)}</span>)}</div>
      {cycleProgress>=37&&<div className="absolute bottom-[17%] left-[27%] text-7xl select-none" aria-hidden>{pick(['🪵','🪨','🍄'],currentCycle)}</div>}
      {cycleProgress>=21&&<div className="absolute left-[53%] top-[42%] animate-[pulse_4s_ease-in-out_infinite] text-4xl select-none" aria-hidden>{pick(['🦋','🐦','🐝','🐞'],currentCycle)}</div>}
      <div className="absolute left-1/2 top-5 w-[min(88%,340px)] -translate-x-1/2 rounded-2xl border border-white/70 bg-white/80 px-5 py-3 text-center shadow-sm backdrop-blur"><p className="text-xs font-medium text-forest-700">{cycleLabel(currentCycle)}</p><p className="mt-0.5 text-sm font-semibold text-forest-900">Nível {level} · {theme.name}</p><p className="mt-1 text-[11px] text-ink-soft">{theme.subtitle}</p></div>
    </section>

    <section className="relative z-10 mx-auto -mt-8 max-w-3xl rounded-[28px] border border-line bg-white/95 p-5 shadow-lg sm:p-7"><div className="flex items-center gap-2" aria-label="Progresso visual do jardim">{[0,1,2,3,4].map(i=><span key={i} className={`h-3 flex-1 rounded-full ${cycleProgress>=i*12?'bg-forest-700':'bg-[#e5dfd4]'}`}/>)}</div><p className="mt-3 text-center text-sm text-ink-soft">{currentCycle>0?`${currentCycle} ${currentCycle===1?'jardim já floresceu':'jardins já floresceram'} na sua história.`:'Você está construindo algo bonito.'}</p><div className="mt-5 flex flex-col gap-4 rounded-2xl bg-[#fbf7ef] p-4 sm:flex-row sm:items-center"><span className="text-5xl" aria-hidden>{latest.icon}</span><div className="flex-1"><p className="text-sm font-semibold text-forest-900">{unlocked.length?`${latest.label.split(' · ')[0]} apareceu no seu jardim`:'Um novo jardim está começando'}</p><p className="mt-1 text-xs leading-5 text-ink-soft">Seu jardim cresce com interações significativas. Quando este jardim florescer por completo, um novo espaço começa automaticamente. Os anteriores continuam fazendo parte da sua história.</p></div><div className="flex gap-2"><button type="button" onClick={()=>setCollection(true)} className="rounded-xl bg-forest-900 px-4 py-2.5 text-xs font-medium text-white">Ver coleção</button>{currentCycle>0&&<button type="button" onClick={()=>{setSelectedCycle(currentCycle-1);setHistory(true)}} className="rounded-xl border border-line bg-white px-4 py-2.5 text-xs font-medium text-forest-900">Jardins anteriores</button>}</div></div></section>
  ...
  <div className="sr-only"><Leaf/><Flower2/><TreePine/> O jardim evolui em ciclos sem limite final. Cada ciclo usa CYCLE_SIZE pontos internos, gera uma nova combinação determinística de plantas, visitantes e detalhes, preserva os jardins concluídos na navegação histórica e não usa streak, punição, perda de nível ou XP visível.</div></div>
}
