  ...
  if (view === 'home') return <div className="max-w-[1180px] mx-auto px-4 sm:px-6 lg:px-8 py-7 sm:py-9 text-ink">
    ...
    <section className="rounded-[24px] border border-line bg-white p-4 sm:p-6">
      ...
      <div className="mt-5 flex border-b border-line" role="tablist" aria-label="Tipos de relatório no histórico">
        <button type="button" role="tab" aria-selected={historyType === 'weekly'} onClick={() => setHistoryType('weekly')} className={`flex-1 sm:flex-none min-w-[190px] inline-flex items-center justify-center gap-2 px-4 py-3 text-sm border-b-2 -mb-px ${historyType === 'weekly' ? 'border-forest-700 text-forest-900 font-medium' : 'border-transparent text-ink-soft'}`}><CalendarDays className="w-4 h-4" /> Relatórios semanais</button>
        <button type="button" role="tab" aria-selected={historyType === 'monthly'} onClick={() => setHistoryType('monthly')} className={`flex-1 sm:flex-none min-w-[190px] inline-flex items-center justify-center gap-2 px-4 py-3 text-sm border-b-2 -mb-px ${historyType === 'monthly' ? 'border-forest-700 text-forest-900 font-medium' : 'border-transparent text-ink-soft'}`}><CalendarCheck2 className="w-4 h-4" /> Relatórios mensais</button>
      </div>
