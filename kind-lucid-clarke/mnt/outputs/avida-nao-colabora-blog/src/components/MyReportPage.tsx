...
export default function MyReportPage(props: Props) {
  ...
  return <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-8 space-y-5">
    ...
    <section className="relative overflow-hidden rounded-[2rem] border border-line bg-gradient-to-r from-mint/55 via-paper-soft to-white p-6 sm:p-8 shadow-sm">
      <div className="relative z-10 max-w-3xl"><p className="text-[11px] uppercase tracking-[0.16em] font-semibold text-forest-600">{isWeekly ? 'Sua semana está pronta' : 'Seu mês está pronto'}</p><h2 className="mt-2 font-serif text-2xl sm:text-3xl text-forest-900">{summary || `Veja o que seus registros contam sobre ${isWeekly ? 'esta semana' : 'este mês'}.`}</h2><p className="mt-2 text-sm text-ink-soft">{periodLabel} · retrospectiva baseada nos registros disponíveis</p><div className="mt-5 flex flex-wrap gap-2"><span className="rounded-full bg-forest-900 text-white px-4 py-2 text-xs font-medium">Resumo</span><button onClick={() => setShowDetails(true)} className="rounded-full border border-line bg-white px-4 py-2 text-xs text-forest-700">Emoções</button><button onClick={() => setShowDetails(true)} className="rounded-full border border-line bg-white px-4 py-2 text-xs text-forest-700">Padrões</button><button onClick={() => setShowDetails(true)} className="rounded-full border border-line bg-white px-4 py-2 text-xs text-forest-700">Conteúdos</button></div></div>
