...
export default function AdminAreaConteudo({ onEditArticle, initialTab, onNavigate }: { onEditArticle:(id?:string)=>void; initialTab?:string; onNavigate?:(view:'estudio-ia'|'site')=>void }) {
 ...
 return <div className="admin-page-pad flex flex-col min-h-0 gap-4">
  <section className="admin-page-hero flex flex-wrap items-start justify-between gap-4"><div><p className="admin-kicker">Biblioteca editorial</p><h1 className="font-serif text-3xl text-forest-900">Conteúdo</h1><p className="admin-subtitle mt-1">Crie, organize e programe os conteúdos publicados no AVNC. IA, SEO e páginas institucionais agora têm áreas próprias.</p></div><div className="admin-actions"><button onClick={()=>onEditArticle()} className="admin-btn-primary"><Plus className="w-4 h-4"/>Novo artigo</button><button onClick={()=>onNavigate?.('estudio-ia')} className="admin-btn-secondary"><Sparkles className="w-4 h-4"/>Criar com IA</button><button onClick={()=>go('calendario')} className="admin-btn-secondary"><CalendarDays className="w-4 h-4"/>Calendário editorial</button><button onClick={()=>onNavigate?.('site')} className="admin-btn-secondary"><LayoutTemplate className="w-4 h-4"/>Site</button></div></section>
