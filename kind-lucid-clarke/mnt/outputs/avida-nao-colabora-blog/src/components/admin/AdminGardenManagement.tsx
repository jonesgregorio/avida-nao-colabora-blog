...
function CampaignPanel({campaigns,gardens,open,setOpen,reload,audit}:{campaigns:Campaign[];gardens:Garden[];open:boolean;setOpen:(v:boolean)=>void;reload:()=>Promise<void>;audit:(a:string,t:string,id?:string,m?:Record<string,unknown>)=>Promise<void>}) {
  const blankCampaign={name:'',garden_slug:'',campaign_type:'launch',audience:'all',headline:'',body:'',cta_label:'Conhecer o jardim',cta_url:'',starts_at:'',ends_at:'',temporary_unlock:false}
  const [form,setForm]=useState(blankCampaign)
  const [busy,setBusy]=useState(false)
  const [formError,setFormError]=useState('')
  async function create(){
    if(!form.name.trim()){setFormError('Preencha o nome da campanha.');return}
    if(!form.headline.trim()){setFormError('Preencha o título — é o que aparece pro usuário.');return}
    setFormError('');setBusy(true)
    const {data,error}=await supabase.from('garden_campaigns').insert({...form,garden_slug:form.garden_slug||null,cta_url:form.cta_url.trim()||null,starts_at:form.starts_at||null,ends_at:form.ends_at||null,status:form.starts_at?'scheduled':'draft'}).select('id').single()
    if(!error&&data){await audit('campaign.create','campaign',data.id,{name:form.name});setOpen(false);setForm(blankCampaign);await reload()}
    else if(error){setFormError('Erro ao salvar: '+error.message)}
    setBusy(false)
  }
  ...
  return <div className="space-y-4"><div className="flex justify-end"><button className="admin-btn-primary" onClick={()=>{setFormError('');setOpen(true)}}><Plus className="h-4 w-4"/>Nova campanha</button></div>{open&&<div className="admin-card p-5"><div className="flex justify-between"><h3 className="font-serif text-xl">Nova campanha de jardim</h3><button onClick={()=>setOpen(false)}><X className="h-4 w-4"/></button></div><div className="mt-4 grid gap-3 md:grid-cols-2"><Input label="Nome" value={form.name} onChange={v=>setForm({...form,name:v})}/><label className="text-xs text-ink-soft">Jardim<select className="mt-1 w-full rounded-xl border border-line bg-white p-2.5 text-sm" value={form.garden_slug} onChange={e=>setForm({...form,garden_slug:e.target.value})}><option value="">Todos / teaser</option>{gardens.map(g=><option key={g.slug} value={g.slug}>{g.label}</option>)}</select></label><label className="text-xs text-ink-soft">Tipo<select className="mt-1 w-full rounded-xl border border-line bg-white p-2.5 text-sm" value={form.campaign_type} onChange={e=>setForm({...form,campaign_type:e.target.value})}><option value="launch">Lançamento</option><option value="featured">Destaque</option><option value="seasonal">Sazonal</option><option value="reactivation">Retorno</option><option value="achievement">Conquista</option><option value="prelaunch">Pré-lançamento</option></select></label><label className="text-xs text-ink-soft">Público<select className="mt-1 w-full rounded-xl border border-line bg-white p-2.5 text-sm" value={form.audience} onChange={e=>setForm({...form,audience:e.target.value})}><option value="all">Todos</option><option value="completed_one">Concluiu pelo menos 1</option><option value="at_100">Está em 100%</option><option value="inactive">Inativos</option><option value="new_users">Novos usuários</option><option value="garden_users">Usuários de um jardim</option></select></label><Input label="Título" value={form.headline} onChange={v=>setForm({...form,headline:v})}/><Input label="Texto" value={form.body} onChange={v=>setForm({...form,body:v})}/><Input label="Texto do botão" value={form.cta_label} onChange={v=>setForm({...form,cta_label:v})}/><Input label="Link do botão (opcional — sem link, aparece só como selo, não clicável)" value={form.cta_url} onChange={v=>setForm({...form,cta_url:v})}/><Input label="Início" type="datetime-local" value={form.starts_at} onChange={v=>setForm({...form,starts_at:v})}/><Input label="Fim" type="datetime-local" value={form.ends_at} onChange={v=>setForm({...form,ends_at:v})}/></div><label className="mt-3 flex items-center gap-2 text-sm"><input type="checkbox" checked={form.temporary_unlock} onChange={e=>setForm({...form,temporary_unlock:e.target.checked})}/>Desbloqueio temporário durante a campanha</label><p className="mt-1 text-[10px] text-ink-soft">Enquanto esta campanha estiver ativa, libera o Meu Jardim mesmo para quem não tem o plano Essencial — assim que a campanha pausar, expirar ou o público-alvo deixar de bater, o acesso volta a exigir o plano.</p>{formError&&<p className="mt-3 text-xs text-red-700">{formError}</p>}<button className="admin-btn-primary mt-4" disabled={busy} onClick={()=>void create()}>{busy?<Loader2 className="h-4 w-4 animate-spin"/>:<Save className="h-4 w-4"/>}Salvar campanha</button></div>}<div className="grid gap-3 md:grid-cols-2">{campaigns.map(c=><article key={c.id} className="admin-card p-5"><div className="flex items-start justify-between"><div><p className="text-[10px] uppercase tracking-wide text-ink-soft">{c.campaign_type} · {c.audience}</p><h3 className="font-serif text-xl text-forest-900">{c.name}</h3></div><div className="flex flex-col items-end gap-1"><span className="rounded-full bg-forest-50 px-2 py-1 text-[10px] text-forest-700">{c.status}</span>{c.temporary_unlock&&<span className="rounded-full bg-amber-50 px-2 py-1 text-[10px] text-amber-800">Desbloqueio temporário</span>}</div></div><p className="mt-2 text-sm">{c.headline}</p><p className="mt-1 text-xs text-ink-soft">{c.body}</p><p className="mt-3 text-xs text-ink-soft">{fmtDate(c.starts_at)} → {fmtDate(c.ends_at)}</p><button className="admin-btn-secondary mt-4" disabled={busy} onClick={()=>void toggle(c)}>{c.status==='active'?'Pausar':'Ativar'}</button></article>)}</div></div>
}
...
function RulesPanel({settings,setSettings,save,saving}:{settings:Settings;setSettings:(s:Settings)=>void;save:()=>Promise<void>;saving:boolean}) {
  return <div className="admin-card max-w-3xl p-6"><h3 className="font-serif text-2xl text-forest-900">Regras de crescimento e distribuição</h3><p className="mt-1 text-sm text-ink-soft">A fila global já está operacional. Livre escolha e híbrido ficam registrados como política para a experiência de seleção do usuário.</p><div className="mt-6 grid gap-4 sm:grid-cols-2"><label className="text-xs text-ink-soft">Modo de entrega<select className="mt-1 w-full rounded-xl border border-line bg-white p-2.5 text-sm" value={settings.release_mode} onChange={e=>setSettings({...settings,release_mode:e.target.value as Settings['release_mode']})}><option value="global">Fila global</option><option value="free">Livre escolha</option><option value="hybrid">Híbrido</option></select></label><label className="text-xs text-ink-soft">Pontos por ciclo<input type="number" className="mt-1 w-full rounded-xl border border-line bg-white p-2.5 text-sm" value={settings.points_per_cycle} onChange={e=>setSettings({...settings,points_per_cycle:Number(e.target.value)})}/></label><label className="text-xs text-ink-soft">Limite diário (opcional)<input type="number" className="mt-1 w-full rounded-xl border border-line bg-white p-2.5 text-sm" value={settings.daily_growth_cap??''} onChange={e=>setSettings({...settings,daily_growth_cap:e.target.value?Number(e.target.value):null})}/><span className="mt-1 block text-[10px] text-ink-soft">Quando definido, o crescimento de cada pessoa não pode subir mais do que esse valor por dia (o que passar do limite fica disponível no dia seguinte).</span></label><label className="text-xs text-ink-soft">Marcos visuais<input className="mt-1 w-full rounded-xl border border-line bg-white p-2.5 text-sm" value={settings.stage_thresholds.join(', ')} onChange={e=>setSettings({...settings,stage_thresholds:e.target.value.split(',').map(v=>Number(v.trim())).filter(Number.isFinite)})}/></label></div><button className="admin-btn-primary mt-6" disabled={saving} onClick={()=>void save()}>{saving?<Loader2 className="h-4 w-4 animate-spin"/>:<Save className="h-4 w-4"/>}Salvar regras</button></div>
}
