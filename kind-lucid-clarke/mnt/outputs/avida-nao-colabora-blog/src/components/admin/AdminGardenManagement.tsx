...
function RulesPanel({settings,setSettings,save,saving}:{settings:Settings;setSettings:(s:Settings)=>void;save:()=>Promise<void>;saving:boolean}) {
  return <div className="admin-card max-w-3xl p-6"><h3 className="font-serif text-2xl text-forest-900">Regras de crescimento e distribuição</h3><p className="mt-1 text-sm text-ink-soft">A fila global já está operacional. Livre escolha e híbrido ficam registrados como política para a experiência de seleção do usuário.</p><div className="mt-6 grid gap-4 sm:grid-cols-2"><label className="text-xs text-ink-soft">Modo de entrega<select className="mt-1 w-full rounded-xl border border-line bg-white p-2.5 text-sm" value={settings.release_mode} onChange={e=>setSettings({...settings,release_mode:e.target.value as Settings['release_mode']})}><option value="global">Fila global</option><option value="free">Livre escolha</option><option value="hybrid">Híbrido</option></select></label><label className="text-xs text-ink-soft">Pontos por ciclo<input type="number" className="mt-1 w-full rounded-xl border border-line bg-white p-2.5 text-sm" value={settings.points_per_cycle} onChange={e=>setSettings({...settings,points_per_cycle:Number(e.target.value)})}/></label><label className="text-xs text-ink-soft">Limite diário (opcional)<input type="number" className="mt-1 w-full rounded-xl border border-line bg-white p-2.5 text-sm" value={settings.daily_growth_cap??''} onChange={e=>setSettings({...settings,daily_growth_cap:e.target.value?Number(e.target.value):null})}/></label><label className="text-xs text-ink-soft">Marcos visuais<input className="mt-1 w-full rounded-xl border border-line bg-white p-2.5 text-sm" value={settings.stage_thresholds.join(', ')} onChange={e=>setSettings({...settings,stage_thresholds:e.target.value.split(',').map(v=>Number(v.trim())).filter(Number.isFinite)})}/></label></div><button className="admin-btn-primary mt-6" disabled={saving} onClick={()=>void save()}>{saving?<Loader2 className="h-4 w-4 animate-spin"/>:<Save className="h-4 w-4"/>}Salvar regras</button></div>
}
