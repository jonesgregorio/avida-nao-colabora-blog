...
function CampaignPanel({campaigns,gardens,open,setOpen,reload,audit}:{campaigns:Campaign[];gardens:Garden[];open:boolean;setOpen:(v:boolean)=>void;reload:()=>Promise<void>;audit:(a:string,t:string,id?:string,m?:Record<string,unknown>)=>Promise<void>}) {
  ...
  const [busy,setBusy]=useState(false)
  async function create(){ if(!form.name.trim())return;setBusy(true);const {data,error}=await supabase.from('garden_campaigns').insert({...form,garden_slug:form.garden_slug||null,cta_url:form.cta_url.trim()||null,starts_at:form.starts_at||null,ends_at:form.ends_at||null,status:form.starts_at?'scheduled':'draft'}).select('id').single();if(!error&&data){await audit('campaign.create','campaign',data.id,{name:form.name});setOpen(false);setForm(blankCampaign);await reload()}setBusy(false)}
