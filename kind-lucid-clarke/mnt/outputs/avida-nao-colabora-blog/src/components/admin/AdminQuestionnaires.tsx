    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      {activeTab === 'settings' && (
        <div className="space-y-4">
          ...
          <button onClick={addQuestion} className="flex items-center gap-2 text-sm text-emerald-600 hover:underline mt-2">
      ...
      {activeTab === 'results' && (
        <div>
          {editing.results.map((r, i) => (
            <div key={r.id} className="border rounded-lg p-4 mb-3">
              <div className="flex items-center gap-2 mb-3">
                <input type="color" value={r.color} onChange={e => updateResult(i, { ...r, color: e.target.value })} className="w-8 h-8 rounded border cursor-pointer" />
                <input className="flex-1 border rounded px-2 py-1 text-sm font-medium" placeholder="Label do resultado" value={r.label} onChange={e => updateResult(i, { ...r, label: e.target.value })} />
                <button onClick={() => removeResult(i)} className="text-red-400 hover:text-red-600"><X size={14} /></button>
              </div>
              <div className="grid grid-cols-2 gap-3 mb-3">
                <div>
                  <label className="text-xs text-stone-500 block mb-1">Pontuação mínima</label>
                  <input type="number" className="w-full border rounded px-2 py-1 text-sm" value={r.min_score} onChange={e => updateResult(i, { ...r, min_score: Number(e.target.value) })} />
                </div>
                <div>
                  <label className="text-xs text-stone-500 block mb-1">Pontuação máxima</label>
                  <input type="number" className="w-full border rounded px-2 py-1 text-sm" value={r.max_score} onChange={e => updateResult(i, { ...r, max_score: Number(e.target.value) })} />
                </div>
              </div>
              <div className="space-y-2">
                <textarea className="w-full border rounded px-2 py-1 text-sm resize-none" rows={2} placeholder="Descrição do resultado" value={r.description} onChange={e => updateResult(i, { ...r, description: e.target.value })} />
                <textarea className="w-full border rounded px-2 py-1 text-sm resize-none" rows={2} placeholder="Recomendação" value={r.recommendation} onChange={e => updateResult(i, { ...r, recommendation: e.target.value })} />
              </div>
            </div>
          ))}
          <button onClick={addResult} className="flex items-center gap-2 text-sm text-emerald-600 hover:underline mt-2">
            <Plus size={14} />Adicionar faixa de resultado
          </button>
        </div>
      )}
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select className="w-full border rounded-lg px-3 py-2 text-sm" value={editing.status} onChange={e => setEditing(x => ({ ...x, status: e.target.value as QStatus }))}>
                {Object.entries(STATUS_CONFIG).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}
              </select>
            </div>
            <div>
              <label className="text-xs text-stone-500 block mb-1">Plano mínimo necessário</label>
              <select className="w-full border rounded-lg px-3 py-2 text-sm" value={editing.plan_required} onChange={e => setEditing(x => ({ ...x, plan_required: e.target.value }))}>
                {PLAN_OPTIONS.map(p => <option key={p.value} value={p.value}>{p.label}</option>)}
              </select>
            </div>
            <div>
              <label className="text-xs text-stone-500 block mb-1">Agendar para</label>
              <input type="datetime-local" className="w-full border rounded-lg px-3 py-2 text-sm" value={editing.scheduled_at} onChange={e => setEditing(x => ({ ...x, scheduled_at: e.target.value }))} />
            </div>
            <div>
              <label className="text-xs text-stone-500 block mb-1">URL da imagem de capa</label>
              <input className="w-full border rounded-lg px-3 py-2 text-sm" value={editing.cover_image} onChange={e => setEditing(x => ({ ...x, cover_image: e.target.value }))} />
            </div>
          </div>
          <div className="space-y-2 pt-2 border-t">
            {[
              { key: 'show_on_questionnaires_page', label: 'Exibir na página pública de questionários' },
              { key: 'show_score',    label: 'Mostrar pontuação ao final' },
              { key: 'show_result',   label: 'Mostrar resultado ao final' },
              { key: 'allow_anonymous', label: 'Permitir resposta anônima' },
              { key: 'allow_retake',  label: 'Permitir refazer o questionário' },
            ].map(({ key, label }) => (
              <label key={key} className="flex items-center gap-2 text-sm cursor-pointer">
                <input
                  type="checkbox"
                  checked={editing[key as keyof QData] as boolean}
                  onChange={e => setEditing(x => ({ ...x, [key]: e.target.checked }))}
                />
                {label}
              </label>
            ))}
