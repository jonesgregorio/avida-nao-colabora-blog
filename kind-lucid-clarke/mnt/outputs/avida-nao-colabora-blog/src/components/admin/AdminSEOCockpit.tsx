...
export default function AdminSEOCockpit({ onEditArticle }: { onEditArticle?: (id: string) => void }) {
  ...
  const cards: { key: Issue; label: string }[] = [
    { key: 'no_seo', label: 'Metadados incompletos' }, { key: 'no_image', label: 'Imagem ou alt ausente' },
    { key: 'bad_slug', label: 'Slug ruim' }, { key: 'thin', label: 'Conteúdo curto' }, { key: 'no_author', label: 'Sem autoria' },
  ...
  async function genSEO(a: Row) {
    ...
    try {
      ...
      else { flash('SEO atualizado para "' + a.title + '".'); await loadArticles() }
  ...
  return <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
    ...
    {tab === 'indexing' && <Indexing rows={dashboard?.inspections || []} />}
...
function Indexing({ rows }: { rows: Inspection[] }) { return <Panel title="Indexação de URLs públicas" subtitle="URL Inspection mostra a versão conhecida pelo Google; não é um teste ao vivo.">{rows.length === 0 ? <Empty text="Nenhuma URL foi inspecionada ainda." /> : <div className="overflow-x-auto"><table className="w-full text-xs sm:text-sm"><thead><tr className="border-b border-line text-left text-ink-soft"><th className="py-2 pr-3">URL</th><th>Estado</th><th>Cobertura</th><th>Inspecionada</th></tr></thead><tbody className="divide-y divide-stone-100">{rows.map(item => <tr key={item.url}><td className="py-3 pr-3 max-w-[420px] truncate"><a href={item.url} target="_blank" rel="noreferrer" className="text-forest-800 hover:underline">{item.url}</a></td><td><Status ok={item.verdict === 'PASS'} label={item.verdict || '—'} /></td><td className="text-ink-soft">{item.coverage_state || '—'}</td><td className="text-ink-soft whitespace-nowrap">{new Date(item.last_inspected_at).toLocaleDateString('pt-BR')}</td></tr>)}</tbody></table></div>}</Panel> }
...
function Opportunities({ rows }: { rows: Opportunity[] }) { return <Panel title="Oportunidades priorizadas" subtitle="Regras transparentes: CTR baixo com impressões, posições 8–20 e páginas visíveis com poucos cliques.">{rows.length === 0 ? <Empty text="Nenhuma oportunidade forte foi detectada." /> : <div className="space-y-3">{rows.map((row, i) => <div key={`${row.type}-${row.subject}-${i}`} className="rounded-xl border border-line p-4"><div className="flex flex-wrap justify-between gap-2"><p className="font-medium text-forest-900 break-all">{row.subject}</p><span className="text-[11px] px-2 py-1 rounded-full bg-mint text-forest-800">{row.reason}</span></div><p className="text-xs text-ink-soft mt-2">{fmt(row.impressions)} impressões · {fmt(row.clicks)} cliques · CTR {pct(row.ctr)} · posição {fmt(row.position)}</p></div>)}</div>}</Panel> }
