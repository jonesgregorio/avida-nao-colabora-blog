...
export default function AdminSEOCockpit({ onEditArticle }: { onEditArticle?: (id: string) => void }) {
  ...
  return <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
    ...
    {tab === 'indexing' && <Indexing rows={dashboard?.inspections || []} articles={publishedRows} busyId={busyId} onCorrect={correctArticle} />}
...
function Opportunities({ rows, articles, busyId, onCorrect }: { rows: Opportunity[]; articles: Row[]; busyId: string | null; onCorrect: (r: Row, issue?: SeoSmartIssue) => Promise<void> }) {
  ...
  return <Panel title="Oportunidades de crescimento" subtitle="Aqui o sistema mostra onde pequenas melhorias podem trazer mais visitas.">{rows.length === 0 ? <Empty text="Nenhuma oportunidade forte foi encontrada agora." /> : <div className="space-y-3">{rows.map((row, i) => { const article = articleFromUrl(row.subject, articles); return <div key={`${row.type}-${row.subject}-${i}`} className="rounded-xl border border-line p-4"><p className="font-medium text-forest-900 break-all">{row.subject}</p><p className="text-xs text-ink-soft mt-1">{reason(row)}</p><div className="flex flex-wrap items-center justify-between gap-3 mt-2"><p className="text-xs text-ink-soft">{fmt(row.impressions)} aparições · {fmt(row.clicks)} cliques · taxa {pct(row.ctr)} · posição média {fmt(row.position)}</p>{article && <button onClick={() => void onCorrect(article, 'opportunity')} disabled={busyId === article.id} className="inline-flex items-center gap-1.5 text-xs border border-forest-200 text-forest-800 px-2.5 py-1.5 rounded-lg disabled:opacity-50">{busyId === article.id ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <WandSparkles className="w-3.5 h-3.5" />} Melhorar com IA</button>}</div></div> })}</div>}</Panel>
}
