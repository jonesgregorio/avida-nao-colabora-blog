...
export default function AdminSavedItems() {
  ...
  useEffect(() => {
    async function load() {
      const { data } = await supabase
        .from('saved_items')
        .select('id, user_id, item_type, item_id, title, description, created_at')
  ...
  const filtered = search
    ? items.filter(i =>
        ...
        i.user_id.slice(0, 8).includes(search)
      )
    : items
  ...
  // Artigos mais salvos (item_type === 'article')
  const byArticle: Record<string, { title: string; count: number }> = {}
  items.filter(i => i.item_type === 'article').forEach(i => {
    const k = i.item_id
    if (!byArticle[k]) byArticle[k] = { title: i.title || i.item_id, count: 0 }
    byArticle[k].count++
  })
  const topArticles = Object.entries(byArticle).sort((a, b) => b[1].count - a[1].count).slice(0, 5)

  const TYPE_LABELS: Record<string, string> = {
    article: 'Artigo', trail: 'Trilha', questionnaire: 'Questionário', content: 'Conteúdo',
  }
              ...
              {filtered.slice(0, 100).map(item => (
                <tr key={item.id} className="hover:bg-stone-50">
                  <td className="px-4 py-3">
                    <p className="text-stone-700 font-medium text-sm truncate max-w-xs">
