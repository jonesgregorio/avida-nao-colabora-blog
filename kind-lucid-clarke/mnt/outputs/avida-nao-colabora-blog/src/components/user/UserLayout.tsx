...
import {
  Home, NotebookPen, BookOpen, Sprout, CreditCard, User as UserIcon, LogOut,
  Shield, ChevronDown, LifeBuoy, Leaf, Bell, Compass, Flower2, CircleUserRound,
} from 'lucide-react'
...
const MAIN_NAV: NavItem[] = [
  ...
  { id: 'diary', label: 'Registrar', shortLabel: 'Registrar', Icon: NotebookPen, match: ['diary'] },
  {
    id: 'descobertas',
    label: 'Evolução',
    Icon: Compass,
    match: ['descobertas', 'my-evolution', 'my-report', 'my-history'],
  },
  {
    id: 'cuidar',
    label: 'Cuidar',
    Icon: Sprout,
    match: ['cuidar', 'self-care', 'monthly-guidance', 'professional-comments', 'questionarios', 'questionnaire', 'questionarios-evolucao'],
    matchPath: ['/plano-de-autocuidado'],
  },
  { id: 'my-garden', label: 'Meu Jardim', shortLabel: 'Jardim', Icon: Flower2, match: ['my-garden'] },
  { id: 'articles', label: 'Conteúdos', Icon: BookOpen, match: ['articles', 'article', 'content'] },
  {
    id: 'mais',
    label: 'Conta',
    Icon: CircleUserRound,
    match: ['mais', 'my-plan', 'profile', 'support', 'support-ticket', 'notifications'],
  },
]

const MOBILE_IDS = ['home', 'diary', 'descobertas', 'cuidar', 'mais'] as const

const HEADER_LABELS: Record<string, string> = {
  home: 'Hoje',
  diary: 'Registrar',
  descobertas: 'Sua Evolução',
...
export default function UserLayout({ user, profile, currentView, onNavigate, onSignOut, children }: UserLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-paper flex">
      <aside className="hidden lg:flex flex-col w-[248px] xl:w-[260px] flex-shrink-0 bg-sand-50 border-r border-line sticky top-0 h-screen overflow-y-auto">
        <SidebarContent name={name} items={MAIN_NAV} isActive={isActive} go={go} />
      </aside>

      <div className="flex-1 min-w-0 flex flex-col">
        <header className="sticky top-0 z-40 bg-paper/95 backdrop-blur border-b border-line">
          <div className="min-h-16 md:min-h-[72px] px-4 sm:px-6 lg:px-8 py-2 flex items-center gap-3">
          ...
          <div className="lg:hidden px-4 sm:px-6 pb-2 -mt-1">
            <p className="text-[11px] font-semibold text-forest-700 truncate">{currentLabel}</p>
          </div>
        </header>
      ...
      <nav aria-label="Navegação principal" className="lg:hidden fixed inset-x-0 bottom-0 z-40 bg-white/95 backdrop-blur border-t border-line shadow-[0_-8px_28px_rgba(15,47,37,0.08)]" style={{ paddingBottom: 'max(0.5rem, env(safe-area-inset-bottom))' }}>
        <div className="grid grid-cols-5 px-1 pt-1.5">
          {MOBILE_IDS.map(id => {
            const item = MAIN_NAV.find(navItem => navItem.id === id)!
...
function SidebarContent({ name, items, isActive, go }: { name: string; items: NavItem[]; isActive: (i: NavItem) => boolean; go: (id: string) => void }) {
  return <div className="flex flex-col min-h-full">
    ...
    <nav className="flex-1 px-3 pb-3" aria-label="Área do usuário">
      <p className="px-3 mb-2 text-[10px] uppercase tracking-[0.14em] font-semibold text-ink-soft/75">Sua jornada</p>
      <div className="space-y-1">{items.map(item => <NavButton key={item.id} item={item} active={isActive(item)} onClick={() => go(item.id)} />)}</div>
      <div className="mx-3 my-5 h-px bg-line" />
      <p className="px-3 text-[11px] leading-relaxed text-ink-soft">Registrar → entender → cuidar. O menu mostra intenções; os detalhes ficam dentro de cada área.</p>
    </nav>
    <div className="p-4 mt-auto"><button onClick={() => go('support')} className="w-full flex items-center justify-between gap-2 rounded-2xl border border-line bg-white px-4 py-3 text-sm font-medium text-forest-900 hover:bg-mint/40 transition-colors"><span className="inline-flex items-center gap-2"><LifeBuoy className="w-4 h-4" />Suporte</span><span aria-hidden>→</span></button></div>
  </div>
}
