...
export interface RecordsSummary {
  ...
  questionnaireSignals?: Record<string, unknown>[]
  avgEnergy: number
  avgAnxiety: number
  avgSleep: number
...
export function buildCarePlanPrompt(rs: RecordsSummary): string {
  const metrics = {
    ...
    sinais_de_questionarios: rs.questionnaireSignals ?? [],
...
function asEmotionalSummary(rs: RecordsSummary): EmotionalSummary {
  ...
  return {
    ...
    questionnaire_signals: { completed_count: rs.questionnaireSignals?.length ?? 0, latest_results: rs.questionnaireSignals ?? [], top_tags: [] },
...
export function fallbackCarePlan(a: EmotionalAnalysis, monthLabel: string): CarePlanResult {
  ...
  return {
    ...
    care_plan: {
      ...
      monthly_priority: 'Escolher um pequeno passo de cuidado possível.',
      main_care: 'Transformar uma observação do mês em uma ação leve e possível.',
      recommended_practice: 'Faça um check-in breve quando fizer sentido.',
      attention_point: 'Observe os momentos que merecem atenção, sem tentar resolver tudo de uma vez.',
      small_commitment: 'Escolha uma ação leve em um dia da semana.',
      checkin_suggestion: 'Registrar como você está antes e depois de uma pausa pode ajudar a perceber o que funciona.',
