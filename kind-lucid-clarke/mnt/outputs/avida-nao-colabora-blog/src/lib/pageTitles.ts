...
const DEFAULT_DESCRIPTION = 'Um espaço para organizar o que você sente, acompanhar seus padrões emocionais e cuidar de si com mais leveza.'
...
const PUBLIC_META: Record<string, { path: string; description: string }> = {
  ...
  guides: { path: '/guias', description: 'Guias sobre diário emocional, emoções, sobrecarga, autocuidado, sono, descanso, relações e limites, organizados em caminhos de leitura.' },
...
const VIEW_TITLES: Record<string, string> = { home: HOME_TITLE, auth: 'Entrar', diary: 'Diário', descobertas: 'Descobertas', 'my-evolution': 'Mapa Emocional', 'my-report': 'Relatórios', 'my-history': 'Minha História', 'my-garden': 'Meu Jardim', 'self-care': 'Plano de Autocuidado', 'monthly-guidance': 'Orientação', 'professional-comments': 'Orientação', articles: 'Blog', guides: 'Guias de bem-estar emocional', 'editorial-policy': 'Política editorial', article: 'Conteúdos Guiados', questionarios: 'Questionários', 'questionarios-evolucao': 'Questionários', questionnaire: 'Questionário', 'my-plan': 'Meu Plano', profile: 'Perfil', support: 'Suporte', 'support-ticket': 'Suporte', notifications: 'Notificações', pricing: 'Planos', mais: 'Mais', cuidar: 'Cuidar', about: 'Sobre', contact: 'Contato', faq: 'Perguntas frequentes', privacy: 'Política de Privacidade', terms: 'Termos de Uso', responsibility: 'Aviso de Responsabilidade', admin: 'Painel Admin', success: 'Assinatura confirmada', 'newsletter-unsubscribed': 'Inscrição cancelada' }
export function titleForView(view: string): string { if (view === 'home') return HOME_TITLE; const label = VIEW_TITLES[view]; return label ? `${label} — ${SITE_NAME}` : HOME_TITLE }
export function applyRouteMetadata(view: string, pathname = window.location.pathname): void {
  if (view === 'article') return
  if (view === 'guides' && pathname.startsWith('/guias/')) return
  const title = titleForView(view)
  document.title = title
  const publicMeta = PUBLIC_META[view]
  const url = `${ORIGIN}${publicMeta?.path ?? pathname}`
  const setMeta = (selector: string, content: string) => { const el = document.head.querySelector(selector); if (el) el.setAttribute('content', content) }
