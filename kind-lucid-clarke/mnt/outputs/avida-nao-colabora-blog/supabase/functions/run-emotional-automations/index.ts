...
function summaryOf(rows: Record<string, unknown>[], start: string, end: string, plan: 'essential' | 'plus') {
  ...
  return {
    ...
    averages: { mood: average(rows.map(r => r.mood_score)), energy: average(rows.map(r => r.energy)), anxiety: average(rows.map(r => r.anxiety_level)), sleep: average(rows.map(r => r.sleep_quality)), stress: plan === 'plus' ? average(rows.map(r => r.stress_level)) : 0, selfEsteem: plan === 'plus' ? average(rows.map(r => r.self_esteem_level)) : 0 },
...
Deno.serve(async (req) => {
  ...
  let body: { mode?: 'weekly' | 'monthly' | 'all'; userId?: string } = {}; try { body = await req.json() } catch { /* cron */ }
  const now = new Date(); const weekEnd = new Date(now); weekEnd.setUTCDate(weekEnd.getUTCDate() - ((weekEnd.getUTCDay() + 6) % 7 + 1)); const weekStart = new Date(weekEnd); weekStart.setUTCDate(weekStart.getUTCDate() - 6)
  const monthEnd = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), 0)); const monthStart = new Date(Date.UTC(monthEnd.getUTCFullYear(), monthEnd.getUTCMonth() - 1, 1))
  ...
  for (const profile of candidates as { user_id: string; plan: string; subscription_status?: string; unlimited_access?: boolean }[]) {
    ...
    for (const job of [{ kind: 'weekly' as const, start: isoDay(weekStart), end: isoDay(weekEnd), allowed: true }, { kind: 'monthly' as const, start: isoDay(monthStart), end: isoDay(monthEnd), allowed: plan === 'plus' }]) {
      ...
      const { data: rows } = await admin.from('diary_entries').select('entry_type,diary_kind,date,created_at,mood,mood_score,energy,anxiety_level,sleep_quality,stress_level,self_esteem_level,emotional_tags,context_tags,need_tags,care_action_tags,trigger_tags').eq('user_id', profile.user_id).gte('date', job.start).lte('date', job.end)
    ...
    if (plan === 'plus' && (!body.mode || body.mode === 'all' || body.mode === 'monthly')) {
      ...
      if (!existingPlan) {
        const { data: rows } = await admin.from('diary_entries').select('entry_type,diary_kind,date,created_at,mood,mood_score,energy,anxiety_level,sleep_quality,stress_level,self_esteem_level,emotional_tags,context_tags,need_tags,care_action_tags,trigger_tags').eq('user_id', profile.user_id).gte('date', isoDay(monthStart)).lte('date', isoDay(monthEnd))
