...
type CarePlanActivity = { total_entries:number; active_days:number; source_counts:Record<string,number>; content_signals:string[] }
async function loadCarePlanActivity(admin: AdminClient,userId:string,start:string,end:string):Promise<CarePlanActivity>{
  const { data,error }=await admin.rpc('care_plan_activity_summary',{p_user:userId,p_start:start,p_end:end})
  if(error) throw new Error(`Falha ao carregar atividade multifuente do Plano de Autocuidado: ${error.message}`)
  const row=(data||{}) as Partial<CarePlanActivity>
  return {total_entries:Number(row.total_entries||0),active_days:Number(row.active_days||0),source_counts:row.source_counts||{},content_signals:Array.isArray(row.content_signals)?row.content_signals.map(String):[]}
}
  ...
  for (const profile of candidates as { user_id: string; plan: string; subscription_status?: string; unlimited_access?: boolean; unlimited_access_until?: string | null; plan_activated_at?: string | null }[]) {
    ...
    if (plan === 'plus' && (!body.mode || body.mode === 'all' || body.mode === 'monthly')) {
      ...
      if (!existingPlan) {
        ...
        const s = { ...baseSummary, total_entries: activity.total_entries, active_days: activity.active_days, source_activity: activity.source_counts, content_signals: activity.content_signals, data_quality: { ...baseSummary.data_quality, has_enough_data: activity.total_entries >= 12 && activity.active_days >= 8, total_entries: activity.total_entries, active_days: activity.active_days, required_entries: 12, required_active_days: 8, message: activity.total_entries >= 12 && activity.active_days >= 8 ? 'Há registros suficientes em diferentes fontes para uma leitura cuidadosa do período.' : 'Ainda não há atividade suficiente nas fontes disponíveis para criar um plano personalizado.' } }
        if (!s.data_quality.has_enough_data) {
          await admin.from('monthly_care_plans').insert({ user_id: profile.user_id, month_reference: isoDay(monthStart), period_start: careStart, period_end: careEnd, available_at: new Date().toISOString(), plan_required: 'plus', status: 'skipped', records_summary: s, ai_summary: null, ai_summary_json: { data_quality: s.data_quality }, care_plan: {}, generated_by_ai: false, generated_at: new Date().toISOString(), ai_prompt_type: 'self_care_plan', ai_prompt_version: PROMPT_VERSION.self_care_plan, model_used: 'not-called-insufficient-data', fallback_used: false, data_quality: s.data_quality, error_message: null, generated_by: actor })
          results.push(`${profile.user_id}:plano:contexto-insuficiente`)
          continue
        }
        ...
        await log(admin, { user_id: profile.user_id, admin_id: actor, content_type: 'self_care_plan', prompt_type: 'self_care_plan', prompt_version: PROMPT_VERSION.self_care_plan, provider: providerFromModel(model), model_used: model, fallback_used: fallback, data_quality: s.data_quality, source_period_start: s.period_start, source_period_end: s.period_end, incident_entity_key: `self_care_plan:u:${profile.user_id}:p:${s.period_start}`, generation_status: fallback ? 'fallback' : 'success', status: fallback ? 'fallback' : 'success', error_msg: errorMessage })
