-- Etapa 2 (Evolução do Admin) — Central da Jornada / Dashboard Operacional.
--
-- Devolve o funcionamento da plataforma num período [p_start, p_end) + um bloco
-- "requer atenção" com o estado ATUAL (não depende do período). SECURITY DEFINER
-- + is_admin() porque as tabelas de atividade têm RLS `auth.uid() = user_id`.
-- Só contagens agregadas; nenhum dado pessoal ou texto livre.
--
-- Não altera get_operational_metrics() (usado pela Saúde do Sistema).
  ...
  select jsonb_build_object(
    ...
    'attention', jsonb_build_object(
      ...
      'notifications_draft', (
        ...
        where coalesce(status,'draft') <> 'sent' and user_id is null
      ),
