-- Hotfix Etapa 1 (2ª coluna) — admin_user_360: 'avg_mood_90d' fazia
-- avg(mood), mas diary_entries.mood é TEXT (rótulo da emoção: 'Alegria',
-- 'Ansiedade'...). O score numérico 1–5 é diary_entries.mood_score (inteiro).
-- avg(text) -> ERRO 42883: a função inteira falhava e a Ficha 360 mostrava
-- 'ainda não está publicada'. Recria idêntica, só trocando mood -> mood_score.
-- Verificado em produção: com a troca o corpo inteiro roda (avg_mood ~3.7).
-- Idempotente.
...
begin
  ...
  select jsonb_build_object(
    ...
    'mapa', jsonb_build_object(
      'avg_mood_90d', (
        select round(avg(mood_score)::numeric, 2) from public.diary_entries
        where user_id = target_user_id and mood_score is not null
    ...
    'subscription', (
      select jsonb_build_object(
        ...
        'stripe_subscription_id', us.provider_subscription_id,
