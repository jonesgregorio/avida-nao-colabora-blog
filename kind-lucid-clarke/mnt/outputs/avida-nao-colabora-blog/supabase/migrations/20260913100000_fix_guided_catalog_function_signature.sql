-- ============================================================================
-- Corrige a aplicação de 20260913000000_guided_content_player.sql, que FALHOU
-- em produção: `CREATE OR REPLACE FUNCTION get_guided_catalog()` não pode
-- mudar a lista de colunas de retorno sem um DROP FUNCTION antes (erro real do
-- Postgres: 42P13, confirmado no log do workflow "Apply Supabase Migrations").
-- Como a chamada à API roda o arquivo inteiro numa transação, a falha reverteu
-- TUDO daquele arquivo — as colunas novas e as duas tabelas novas também não
-- chegaram a existir. Esta migration repete o conteúdo inteiro (idempotente:
-- ADD COLUMN IF NOT EXISTS / CREATE TABLE IF NOT EXISTS / DROP+CREATE POLICY)
-- e resolve o único ponto que não era idempotente com um DROP FUNCTION antes.
-- ============================================================================
...
-- 2. Etapas de um conteúdo guiado.
...
-- 3. Progresso de cada pessoa em cada conteúdo guiado.
...
-- 4. Catálogo público — precisa do DROP explícito porque a lista de colunas de
--    retorno mudou (é isso que causou a falha original: erro 42P13).
DROP FUNCTION IF EXISTS public.get_guided_catalog();
