...
begin
 ...
 signals as (
   ...
   union select title from personalized_content_deliveries where user_id=p_user and coalesce(read_at,sent_at,created_at)::date between p_start and p_end and title is not null
...
comment on function public.care_plan_activity_summary(uuid,date,date) is 'Fonte canônica de elegibilidade e sinais multifuente do Plano de Autocuidado para Admin e automação.';
commit;