...
test('artigos mantêm ArticleView como dono do título e usam fallback seguro durante o carregamento',()=>{assert.match(titles,/if \(view === 'article'\) return/);assert.match(article,/ARTICLE_FALLBACK_TITLE/);assert.match(article,/const title = \(article\.seo_title \|\| article\.title \|\| 'Artigo'\)\.trim\(\)/)})
