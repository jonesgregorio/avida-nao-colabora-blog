...
test('Meu Jardim traz explicação editorial simples no lugar',()=>{assert.match(garden,/Pequenos cuidados deixam marcas/);assert.match(garden,/Jardins completos viram memórias e um novo sempre começa/);assert.match(garden,/Não existe último jardim por aqui/)})
