...
test('artigos recebem fallback de título imediatamente e ArticleView assume o título específico quando carrega',()=>{assert.match(titles,/view === 'article' \? ARTICLE_FALLBACK_TITLE : titleForView\(view\)/);assert.match(article,/ARTICLE_FALLBACK_TITLE/);assert.match(article,/const title = \(article\.seo_title \|\| article\.title \|\| 'Artigo'\)\.trim\(\)/)})
