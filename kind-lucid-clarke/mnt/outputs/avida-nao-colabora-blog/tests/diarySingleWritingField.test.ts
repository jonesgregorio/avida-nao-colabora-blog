...
test('Diário mantém editor principal, voz, ajuda e detalhes essenciais em camadas progressivas', () => {
  ...
  assert.match(detailsDrawer, /title="Sentimentos principais"/)
  assert.match(detailsDrawer, /open=\{feelingsOpen\}/)
  assert.match(detailsDrawer, /title="O que apareceu por aí\?"/)
  assert.match(detailsDrawer, /TagGroup title="Onde isso apareceu\?"/)
  assert.match(detailsDrawer, /Contexto do dia/)
  assert.doesNotMatch(detailsDrawer, /Aprofundar sinais/)
  assert.doesNotMatch(detailsDrawer, /Gatilhos que você reconhece/)
})
