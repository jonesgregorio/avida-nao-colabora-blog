...
test('garden has no terminal cycle and creates a new garden automatically',()=>{
 ...
 assert.match(ui,/novo começa automaticamente/)
...
test('garden uses a detailed layered scene with visible fauna and depth',()=>{
 ...
 assert.match(ui,/garden-bird-one/)
...
test('garden animates only living or naturally moving elements as they unlock',()=>{
 assert.match(ui,/@keyframes garden-tree-sway/)
 assert.match(ui,/@keyframes garden-flower-sway/)
 assert.match(ui,/@keyframes garden-bird-flight-a/)
 assert.match(ui,/@keyframes garden-butterfly-flight/)
 assert.match(ui,/@keyframes garden-ground-bird-walk/)
 assert.match(ui,/@keyframes garden-duck-swim/)
 assert.match(ui,/@keyframes garden-water-ripple/)
 assert.match(ui,/@keyframes garden-reed-sway/)
 assert.match(ui,/@keyframes garden-leaf-fall/)
 assert.match(ui,/@keyframes garden-firefly/)
 assert.match(ui,/stage >= 2 &&[\s\S]*garden-flower/)
 assert.match(ui,/stage >= 3 &&[\s\S]*garden-tree/)
 assert.match(ui,/stage >= 4 &&[\s\S]*garden-bird-one[\s\S]*garden-ground-bird/)
 assert.match(ui,/stage >= 5 &&[\s\S]*garden-water-ripple/)
 assert.match(ui,/stage >= 5 &&[\s\S]*garden-duck/)
 assert.match(ui,/stage >= 6 &&[\s\S]*garden-firefly/)
