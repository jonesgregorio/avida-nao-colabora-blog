...
import { getEffectivePlan, isContentLocked } from '../src/lib/officialPlans.ts'
import { clearPendingAction, getPendingAction, setPendingAction } from '../src/lib/pendingAction.ts'
...
test('matriz completa do paywall preserva acesso público, conta, Essencial e Plus', () => {
  ...
  for (const [label, required, plan, loggedIn, locked] of cases) assert.equal(isContentLocked(required, plan, loggedIn), locked, label)
})
...
test('acesso ilimitado ativo recebe a regra efetiva de Plus, e o expirado não', () => {
  ...
  assert.equal(active, 'plus'); assert.equal(isContentLocked('plus', active, true), false); assert.equal(expired, 'free'); assert.equal(isContentLocked('plus', expired, true), true)
})

test('visitante de artigo protegido guarda o slug e o App retorna ao mesmo artigo após login', () => {
  const original = Object.getOwnPropertyDescriptor(globalThis, 'sessionStorage')
  const memory = new Map<string, string>()
  Object.defineProperty(globalThis, 'sessionStorage', { configurable: true, value: { getItem: (key: string) => memory.get(key) ?? null, setItem: (key: string, value: string) => memory.set(key, value), removeItem: (key: string) => memory.delete(key) } })
  try { setPendingAction({ view: 'article', articleSlug: 'conteudo-restrito' }); assert.deepEqual(getPendingAction(), { view: 'article', articleSlug: 'conteudo-restrito' }); clearPendingAction(); assert.equal(getPendingAction(), null) }
  finally { if (original) Object.defineProperty(globalThis, 'sessionStorage', original); else delete (globalThis as { sessionStorage?: unknown }).sessionStorage }
...
test('teaser de artigo protegido não projeta o corpo e slug inexistente continua 404', () => {
  ...
  assert.match(articleView, /supabase\.rpc\('get_article_teaser', \{ p_slug: currentSlug \}\)/)
  assert.match(articleView, /row\?\.plan_required && row\.plan_required !== 'free'/)
  assert.match(articleView, /if \(!article\) return[\s\S]*Artigo não encontrado\./)
})
