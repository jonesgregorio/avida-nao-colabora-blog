...
test('22.5 apresenta a retrospectiva narrativa antes dos detalhes', () => {
  ...
  assert.match(wrapper, /O que mudou/)
  assert.match(wrapper, /Padrão da semana/)
  assert.match(wrapper, /Algo para observar/)
  assert.match(wrapper, /Explorar relatório completo/)
})
...
test('22.5 preserva o painel completo atrás do aprofundamento voluntário', () => {
  ...
  assert.match(wrapper, /Explore os dados do período por blocos/)
