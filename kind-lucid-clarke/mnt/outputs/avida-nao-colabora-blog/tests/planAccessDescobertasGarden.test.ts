...
test('MyGardenPage aplica gate Essencial como defesa própria',()=>{assert.match(garden,/hasPlanAccess\(getEffectivePlan\(profile\),\s*'essential'\)/);assert.match(garden,/if\s*\(\s*!access\s*\)/);assert.match(garden,/Disponível a partir do plano Essencial/i)})
